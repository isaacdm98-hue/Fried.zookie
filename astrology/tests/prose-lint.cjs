...
(async () => {
  ...
  for (const p of passages) {
    ...
    for (const s of sents) {
      if (/’(re|s|m|ll|ve|t)\b/.test(s)) continue;   // a contraction carries its finite verb (you’re, it’s, don’t)
      if (s.length > 24 && s.indexOf(':') < 0 && !/\b(is|are|was|were|has|have|had|does|do|did|keeps|stands|answers|gives|comes|come|runs|run|reads|holds|watches|brings|falls|sits|shows|takes|makes|turns|means|pays|leans|carries|carry|define|defines|defend|defends|can|will|may|would|tends|be|been|weighs|scores|rules|meets|opens|puts|works|speaks|arrives|occupies|seconds|presses|underwrites|withholds|counts|marks|earns|won|carried|met|drawn|read|reads|lie|lies|sit|stand|favours|disturbs|troubling|answer|strains|strain|pay|leans|steps|step|advances|advance|belongs|belong|governs|govern|reaches|reach|lifts|lift|builds|build|test|tests|fight|fights|flare|flares|move|moves|earn|earns|need|needs|keep|keeps|collect|collects|find|finds|face|faces|climb|climbs|walk|walks|bloom|blooms|feel|feels|trust|trusts|reward|rewards|pour|pours|hit|hits|dock|docks|argue|argues|get|gets|generate|explains|explain|recover|recovers|guard|guards|do|repay|repays|admit|admits|want|wants|know|knows|stay|stays|grow|grows|wander|wanders|multiply|multiplies|help|helps|hide|hides|open|opens|like|likes|bend|bends|declare|declares|march|marches|announce|announces|strike|strikes|score|scores|shine|shines|notice|notices|matter|matters|burn|burns|bind|binds|flow|flows|seek|seeks|land|lands|ride|rides|recur|recurs|settle|settles|answer|answers|drain|drains|pull|pulls|fill|fills|suit|suits|touches|touch|leaks|leak|repairs|repair|tend|arrive|gets|get|decides|decide|speaks|speak|hold|holds)\b/i.test(s)) flag('fragment', s.slice(0, 80));
    }
